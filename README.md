...
Name: Tandin Zangmo | Student no.: 02230305 | Cluster: kind `dso202` (Kubernetes v1.36.1)
Images: `sarojsanyasi/dso202-{frontend,backend,db}:1.0`

## 1. Architecture note (Task 1)
TODO (about half a page, your own words):
- Which control-plane components act when a Pod is created (API server, etcd, controller manager, scheduler), and which node components run it (kubelet, container runtime, kube-proxy).
- Which objects each tier uses and why (database: Deployment + PVC + headless Service; backend: Deployment + ClusterIP Service; frontend: Deployment + NodePort Service; plus Namespace, ConfigMap, Secret).
...
`DB_NAME` = `POSTGRES_DB`, `DB_USER` = `POSTGRES_USER`, `DB_PASSWORD` = `POSTGRES_PASSWORD`, because the backend and the official Postgres image use different variable names for the same values.

**Secret caveat:** TODO (state that Secrets are only base64-encoded, not encrypted at rest by default, and what that means for who can read them).

## 3. Resource governance (Task 6)
| Tier | CPU req | Mem req | CPU limit | Mem limit |
|---|---|---|---|---|
...
ResourceQuota: requests.cpu 500m, requests.memory 1Gi, limits.cpu 2, limits.memory 1536Mi, pods 10, PVCs 2, requests.storage 2Gi, nodeports 1, loadbalancers 0.
LimitRange (Container): default request 50m/64Mi, default limit 200m/128Mi, min 10m/16Mi, max 500m/512Mi.
Observed quota usage matched the totals above (175m / 352Mi / 850m / 704Mi, 3 Pods, 1 PVC, 1 NodePort).

**Justification:** TODO (why each tier got its size, why the quota has headroom, why nodeports=1 and loadbalancers=0, why the LimitRange max equals the database limit).

## 4. Verification (Task 7)
Raw transcripts are in the appendix and in `evidence/`.
- **7a CRUD:** TODO (what each command did and what it returned).
- **7b DNS:** TODO (curl by Service name from the frontend Pod, and what the nslookup NXDOMAIN lines are).
- **7c Self-healing and persistence:** TODO (backend Pod deleted and recreated by its ReplicaSet; database Pod also deleted, PVC stayed Bound, data survived; why Recreate strategy).
- **7d Declarative vs imperative:** TODO (compare `kubectl apply -f backend/service.yaml` with `kubectl expose`, using the diff: annotation, port name, identifiers).

## 5. Issues encountered
- Published images were linux/arm64 only (`docker buildx imagetools inspect` output), so pulls failed on amd64 with "no match for platform in manifest". I built the unmodified tutor source locally with the same image names and tag 1.0 and loaded it with `kind load docker-image`. Tutor informed: TODO yes/no.
- The frontend's app.js calls `${BACKEND_URL}/api/...` from the browser and nginx.conf has no `/api` proxy, so with BACKEND_URL set to the cluster-internal address the UI shows "Backend unreachable". CRUD was demonstrated via port-forward instead.
- kind nodes are Docker containers and stop when the machine restarts (`docker start dso202-control-plane`, then `kind export kubeconfig --name dso202`).
